...
public class DefaultLayouter implements ILayouter {
	 ...
	 * @return
	 */
	public boolean layout(Point offset, final Word word,
			final Rectangle cloudArea, CloudMatrix mainTree) {
		...
		for (int i = 0; i < 5000; i++) {
			final double radius = Math.sqrt((double) (next.x * next.x + next.y
					* next.y))
					+ growFactor;
			...
			if (cloudArea.x <= word.x && cloudArea.y <= word.y
					&& cloudArea.x + cloudArea.width >= word.x + word.width
					&& cloudArea.y + cloudArea.height >= word.y + word.height) {
				if (rt.fits(mainTree)) {
					rt.place(mainTree, word.id, word);
					return true;
				}
			}
		}
	...
	public void setOption(String optionName, Object object) {
		if (X_AXIS_VARIATION.equals(optionName)) {
			...
			Assert.isLegal(value >= 0,
					"Parameter must be between 0 and 100 (inclusive): " + value);
			Assert.isLegal(value <= 100,
					"Parameter must be between 0 and 100 (inclusive): " + value);
